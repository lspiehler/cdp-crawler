...
let addNetworks = function(snmp) {
    ...
    for(let i = 0; i < snmp.networks.length; i++) {
        ...
        if(networks.hasOwnProperty(netindex)) {
            ...
            network.location[snmp.location] = {
                name: [snmp.name],
                ip: [snmp.networks[i].ip]
            }
            //console.log(network);
    ...
    if(status.running.length < concurrency && status.waiting.length > 0) {
        ...
        if(status.waiting.length < 1 && status.running.length < 1) {
            ...
            for(let i = 0; i < keys.length; i++) {
                ...
                if(arp.hasOwnProperty(keys[i])) {
                    let arpkeys = Object.keys(arp[keys[i]]);
                    networks[keys[i]].hosts = arpkeys.length;
                }
...
let addDevice = function(status, result) {
    ...
    if(config.DBHOST) {
        ...
        let values = [device.hostname, device.status, device.ip, device.stackmembercount, device.snmpuser, device.snmpversion, device.location, device.description, device.platform, device.deviceId, device.discoveredBy, device.sshresult, device.sshusername, device.sshhandshake];
